...
def get_traffic_info():
    ...
    camera_filter_xml = ""
    if county_number_filter is not None:
         camera_filter_xml = f'<FILTER>\n    <EQ name="CountyNo" value="{county_number_filter}" />\n  </FILTER>'
         logging.debug(f"Filtering ALL camera types by County Number: {county_number_filter}")
    else:
         camera_filter_xml = "<FILTER />" # Tomt filter om inget län valts

    # --- Hela XML-frågan med tre queries ---
    xml_query = f"""
<REQUEST>
...
"""
    try:
        ...
        logging.info(f"Successfully fetched data (incl. Camera) from Trafikverket for endpoint /api/traffic-info")
