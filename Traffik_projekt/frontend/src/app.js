...
createApp({
    ...
    setup() {
        ...
        const counties = ref([
            { name: 'Alla län', value: '', number: null, coords: [62.0, 15.0], zoom: 5 },
        ...
        const fetchAndRenderNewData = async (countyValue) => {
            ...
            if (success && data) {
                ...
                const renderStatus = renderMarkersOnMap(
                    data, 
                    filterShowAccidents.value, // Skicka med nytt filter
                    filterShowRoadworks.value, // Skicka med nytt filter
        ...
        watch(selectedCounty, (newValue) => {
            console.log(`Selected county changed to ${newValue}`);
            centerMapOnCounty(newValue);
            fetchAndRenderNewData(newValue);
        });

        // Bevaka ändringar i ALLA filterstatusar
        watch([filterShowAccidents, filterShowRoadworks, filterShowCameras], () => {
            console.log(`Filter changed: Accidents: ${filterShowAccidents.value}, Roadworks: ${filterShowRoadworks.value}, Cameras: ${filterShowCameras.value}`);
        ...
        return {
            ...
            filterShowAccidents, // Exponera nytt filter
            filterShowRoadworks, // Exponera nytt filter
            filterShowCameras
        };
    }
}).mount("#app");
