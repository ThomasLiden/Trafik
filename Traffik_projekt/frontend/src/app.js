...
createApp({
    ...
    setup() {
      ...
      // selectedCounty startar som tomt (ingen localStorage här)
      const selectedCounty = ref(''); 

      // Reaktiv property för meddelande om trafikstatus (för User Story 5)
      ...
      // Funktion för att hämta och visa data, samt uppdatera statusmeddelande
      const fetchDataAndUpdateStatus = async (countyValue) => {
          const status = await updateMapWithTrafficData(countyValue); // updateMapWithTrafficData från map.js
          if (status && status.message) {
              trafficStatusMessage.value = status.message;
          } else {
              trafficStatusMessage.value = "Kunde inte avgöra trafikstatus.";
          }
      };

      watch(selectedCounty, (newValue, oldValue) => {
          console.log(`Selected county changed from ${oldValue} to ${newValue}`);
          // Ingen localStorage-logik här längre
          centerMapOnCounty(newValue);
          fetchDataAndUpdateStatus(newValue);
      });

      onMounted(() => {
          console.log("Vue app mounted, initializing Leaflet...");
          initMap('map');
          fetchDataAndUpdateStatus(selectedCounty.value); // Hämta initial data

          // User Story 3: Automatisk uppdatering varje minut
          setInterval(() => {
              console.log("Automatisk uppdatering av trafikdata...");
              fetchDataAndUpdateStatus(selectedCounty.value);
          }, 1 * 60 * 1000); // 1 minut = 60 000 millisekunder
      });
      ...
      return {
          ...
          selectedCounty,
          counties,
          trafficStatusMessage // Exponera för template (User Story 5)
      };
  }
}).mount("#app");